...
{#if open}
	<div class="fixed top-0 left-0 right-0 bottom-0 bg-secondary-darkest z-50 opacity-95"></div>
	<div
		tabindex="-1"
		aria-hidden="true"
	...
	>
		<div class="relative p-4 w-full max-w-2xl max-h-full">
			<div class="relative bg-white rounded-lg shadow dark:bg-secondary-800">
				...
				>
					<h3 class="text-xl font-semibold text-secondary-darkest dark:text-white">
						{title}
					</h3>
					<Button
						on:click={toggleOpen}
						class="bg-transparent hover:bg-transparent !text-secondary-base hover:text-secondary-dark dark:text-secondary-lightest dark:hover:text-secondary-light !px-0 focus:!ring-0"
					>
